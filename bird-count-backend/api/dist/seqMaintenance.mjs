...
// src/seqMaintenance.ts
import { QueryCommand as QueryCommand2, UpdateCommand as UpdateCommand2 } from "@aws-sdk/lib-dynamodb";
...
async function raiseOnlySet(client, key, value) {
  const r = await client.eval(LUA_RAISE_ONLY_SET, 1, key, String(value));
  return r === 1;
}
...
var PAGE_SIZE = 500;
async function* scanByUpdatedAt(doc, pk) {
  let lastKey;
  do {
...
async function queryDynamoMax(doc, pk) {
  const res = await doc.send(
    new QueryCommand2({
      TableName: TABLE2,
  ...
  return items.length > 0 ? items[0].observationNumber : 0;
}
async function runStatus(doc, trip) {
...
async function runSeed(doc, trip, dryRun, knownMax) {
  const key = tripSeqKey(trip);
  const valkey = valkeyClient();
  const [raw, dbMax] = await Promise.all([
    valkey.get(key),
    knownMax !== void 0 ? Promise.resolve(knownMax) : queryDynamoMax(doc, trip)
  ]);
...
async function runBackfill(doc, trip, dryRun) {
...
async function handler(event) {
  if (!TABLE2) throw new Error("TABLE_NAME is not set");
